...
from ugra.application.use_cases import RouteMessageUseCase, SearchJobsUseCase
...
def _job_card_keyboard(job_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="Открыть", url=f"https://example.com/job/{job_id}")],
        ]
    )
...
async def cmd_jobs(message: Message):
    ...
    for job in jobs[:5]:
        ...
        await message.answer(text, parse_mode="Markdown", reply_markup=_job_card_keyboard(job["id"]))
...
async def cmd_interview(message: Message):
    ...
    response = await use_case.execute(
        session["user_id"],
        "/interview",
        {"job": jobs[0], "skills": session["skills"]},
    )
    await message.answer(response.content, parse_mode="Markdown")
...
async def handle_text(message: Message):
    ...
    response = await use_case.execute(
        session["user_id"],
        message.text,
        {"skills": session["skills"], "experience_years": session["experience_years"]},
    )
    await message.answer(response.content, parse_mode="Markdown")
