...
class GenerateCoverLetterUseCase:
    ...
    async def execute(self, job: dict, resumes: list[dict], experience: str = "") -> str:
        if not resumes:
            resumes = [{"title": "Default", "content": experience or "Опытный специалист"}]
        best = resumes[0] if len(resumes) == 1 else await self._select_resume(resumes, job)
        return await self._cover_letter.generate(
            job=job,
            resume_content=best.get("content", experience),
            user_experience=experience,
        )

    async def _select_resume(self, resumes: list[dict], job: dict) -> dict:
        from ugra.domain.entities import JobSource, JobVacancy

        try:
            job_id = job.get("id")
            vacancy = JobVacancy(
                id=UUID(str(job_id)) if job_id else uuid4(),
                title=job.get("title", ""),
            ...
            picked = await self._resume.select_best(resumes, vacancy)
            return picked or resumes[0]
        except Exception:
            return resumes[0]
