...
from ugra.application.candidate.context_provider import CandidateContextProvider
from ugra.application.candidate.profile_service import CandidateProfileService
from ugra.application.resume.analysis_service import ResumeAnalysisService
...
from ugra.infrastructure.persistence.repositories.candidate_profile_repository import (
    InMemoryAnalysisJobStore,
    InMemoryCandidateProfileRepository,
    InMemoryResumeFileRepository,
)
...
def _create_job_source_registry(settings: Settings) -> JobSourceRegistry:
    ...
    registry.register(GeekJobAdapter())
    return registry
...
class Container(containers.DeclarativeContainer):
    ...
    candidate_profile_repository = providers.Singleton(InMemoryCandidateProfileRepository)
    resume_file_repository = providers.Singleton(InMemoryResumeFileRepository)
    analysis_job_store = providers.Singleton(InMemoryAnalysisJobStore)
    resume_file_storage = providers.Singleton(ResumeFileStorage, settings=config)
    candidate_context_provider = providers.Singleton(
        CandidateContextProvider,
        profile_repo=candidate_profile_repository,
    )
    candidate_profile_service = providers.Singleton(
        CandidateProfileService,
        profile_repo=candidate_profile_repository,
    )
    resume_analysis_service = providers.Singleton(
        ResumeAnalysisService,
        llm=llm_service,
        file_storage=resume_file_storage,
        resume_repo=resume_file_repository,
        profile_repo=candidate_profile_repository,
        job_store=analysis_job_store,
        event_bus=event_bus,
    )
    ...
    career_agent = providers.Singleton(
        ...
        candidate_context=candidate_context_provider,
    )
