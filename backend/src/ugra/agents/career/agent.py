...
class CareerAgent(IntelligenceAgent):
    ...
    async def execute(self, ctx: AgentRuntimeContext) -> AgentResponse:
        user_skills, user_experience = await self._resolve_user_skills(ctx.user_id, ctx.metadata)
...
"""
    ...
    async def analyze_single(self, job: JobVacancy, user_id: UUID, skills: list[str], exp: int) -> dict:
        resolved_skills, resolved_exp = await self._resolve_user_skills(
            user_id, {"skills": skills, "experience_years": exp}
        )
        return await self._analyze_job(job, resolved_skills, resolved_exp, user_id)

    async def _resolve_user_skills(self, user_id: UUID, metadata: dict) -> tuple[list[str], int]:
        if self._candidate_context:
            profile = await self._candidate_context.get_profile(user_id)
            if profile:
                return profile.top_skills(), profile.experience_years()
        return metadata.get("skills", []), int(metadata.get("experience_years", 0))
